...
class ATTACKIndex:
        ...
        for domain in Domain:
            ...
            for tech in loader.get_techniques(domain):
                ...
                for plat in tech.platforms:
                    self._by_platform.setdefault(plat.lower(), []).append(
                        tech.attack_id
                    )
    ...
    def by_tactic(self, tactic_id: str) -> List[TechniqueOrSub]:
        ...
        return [
            self._by_id[tid]
            for tid in self._by_tactic.get(key, [])
    ...
    def by_platform(self, platform: str) -> List[TechniqueOrSub]:
        return [
            self._by_id[tid]
            for tid in self._by_platform.get(platform.lower(), [])
            if tid in self._by_id
        ]
    ...
    def count_techniques(self, domain: Optional[Domain] = None) -> int:
        return sum(
            1
            for t in self._by_id.values()
            if not t.is_subtechnique and (domain is None or t.domain == domain)
        )
    ...
    def count_subtechniques(self, domain: Optional[Domain] = None) -> int:
        return sum(
            1
            for t in self._by_id.values()
            if t.is_subtechnique and (domain is None or t.domain == domain)
        )
