...
class ATTACKIndex:
    ...
    def parent_for(self, technique: TechniqueOrSub) -> Optional[TechniqueOrSub]:
        if not isinstance(technique, SubTechnique):
            return technique
        parent_id = technique.parent_id
        if not parent_id:
            return None
        return self.get_exact(parent_id, technique.domain)
    ...
    def get_subtechniques_of(
        ...
        results: List[SubTechnique] = []
        for candidate_domain in domains:
            for subtechnique_id in self._subtechniques_by_parent.get(
                (candidate_domain, resolved_id), []
            ):
                candidate = self._by_domain_id.get((candidate_domain, subtechnique_id))
                if isinstance(candidate, SubTechnique):
