...
def cmd_lookup(args: argparse.Namespace) -> int:
    ...
    if technique is None:
        ...
        if replacement:
            print(
                f"Technique {args.technique_id} was revoked in v19. "
    ...
    if was_revoked:
        lines.append(
            f"  [!] {args.technique_id} was REVOKED -> resolved to {resolved_id}"
        )
    ...
    if desc:
        ...
        wrapped = textwrap.fill(desc[:500], width=70, initial_indent="    ", subsequent_indent="    ")
...
def cmd_revoked(_args: argparse.Namespace) -> int:
    ...
    print(
        ...
        f"but had sub-techniques added in v19."
    )
    return 0
...
def cmd_navigator(args: argparse.Namespace) -> int:
    ...
    layer = {
        ...
        "description": (
            f"Navigator layer generated by attack-v19-core CLI. "
            f"v19: TA0005=Stealth, TA0112=Defense Impairment."
        ),
        "filters": {
            "platforms": (
                ["Windows", "macOS", "Linux", "AWS", "Azure", "GCP",
                 "Containers", "Network", "SaaS", "IaaS",
